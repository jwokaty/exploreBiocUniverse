...
for (i in seq_along(ru_info)) {
  ...
  windows_builds <- tibble::add_row(windows_builds,
                                    ...
                                    File = ifelse(ru_commit != "",
                                                  paste0(path, Package, "_", binariesversion, ".zip"),
                                                  ""))
}
...
mismatch <- windows_builds |> filter(BBS_commit != RU_commit)
...
The following packages are missing binaries. Why? Are these are packages that have failed to build because Windows isn't supported? Or are they just missing dependencies to be properly built in R Universe?
...
viable_candiates <- windows_builds |>
    ...
    dplyr::filter(BinariesCheck %in% c("NOTE", "WARNING", "OK"))
...
After removing packages missing binaries (see above), the following packages are failing on Windows.
...
failing_windows <- windows_builds |>
    ...
    dplyr::select(Package, Version, JobUrl, BinariesCheck, BinariesStatus,
                  BinariesBuildDate, RU_commit, BBS_commit, JobCheck, File)
...
For more details about the error, click on the JobUrl. It may not take you to the specific location of the error. You need to look at the `Build R-release for Windows` (on left). You can then either expand the `Annotations` or expand `R CMD check` to see more details about the error. You must be logged in to see more details.
