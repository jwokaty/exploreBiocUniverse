...
Are the same packages in bioc.r-universe.dev and 3.23 bbs?
...
```{r find-all-windows-builds, message = FALSE}
library(tibble)
r_version <- "4.6.0"
windows_branch <- "windows-devel"
...
for (i in seq_along(ru_info)) {
  ...
  for (j in seq_along(ru_info[[i]]$`_jobs`)) {
      ...
      if (job$config == windows_branch && job$r == r_version) {
  ...
  path <- "https://bioc.r-universe.dev/bin/windows/contrib/4.6/"
