...
unmatched_ru_builds <- ru_builds |>
    ...
    dplyr::select(Package, Version, RemoteSha) |>
    dplyr::rename(RU_Version = Version, RU_commit = RemoteSha)
...
condensed_builds <- builds |>
    dplyr::rename(JobCheck = `_jobs_check`, BinariesCheck = `_binaries_check`,
                  BinariesUrl = `_binaries_buildurl`, RU_commit = RemoteSha) |>
...
uni <- "bioc-release"
ru_builds <- biocUniTools::get_binaries_by_os(uni, uni_os_branch, r_version, os)
bbs_pkgs_i <- BiocPkgTools::biocBuildReport(version = bioc_version,
                                            pkgType = "software")
```
