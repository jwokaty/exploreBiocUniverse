...
This document attempts to track Windows builds for Bioconductor packages for
the devel and release branches:
...
Because R universe build for multiple versions of R whereas Bioconductor branches
build for a specific version, the following tables display only the package
builds corresponding the Bioconductor branch's specific R version.

A URL may be generated to download a repository.
...
The following packages have the same commit hash in R-Universe and the BBS. 
They also pass the latest `R CMD check` and their binary also passes
`R CMD check` with a value of `NOTE`, `WARNING`, or `OK`.
...
## Bioconductor Devel: URL to download a snapshot of above packages

```{r devel-download-repo-for-passing-packages}
devel_ss <- make_snapshot_url("bioc", "4.6", "win",
                              viable_devel_candidates$Package)
devel_ss
```
...
## Bioconductor Release: URL to download a snapshot of above packages

```{r release-download-repo-for-passing-packages}
release_ss <- make_snapshot_url("bioc", "4.5", "win",
                                viable_release_candidates$Package)
release_ss
```
